...
public class ResultService(IResultRepository resultRepository, ITeamRepository teamRepository, IHubContext<ResultsHub> hubContext) : IResultService
{
    ...
    {
        return (await resultRepository.GetAllAsync()).Select(r => new ResultDto(r.Id, r.TeamId, r.Team?.Name ?? "", r.DisciplineId, r.Points, r.RawValue, r.UpdatedAt));
    }

    public async Task<IEnumerable<ResultDto>> GetByDisciplineAsync(string disciplineId)
    {
        return (await resultRepository.GetByDisciplineAsync(disciplineId)).Select(r => new ResultDto(r.Id, r.TeamId, r.Team?.Name ?? "", r.DisciplineId, r.Points, r.RawValue, r.UpdatedAt));
    }

    public async Task<IEnumerable<ResultDto>> GetByTeamAsync(Guid teamId)
    {
        return (await resultRepository.GetByTeamAsync(teamId)).Select(r => new ResultDto(r.Id, r.TeamId, r.Team?.Name ?? "", r.DisciplineId, r.Points, r.RawValue, r.UpdatedAt));
    }
    ...
    public async Task<ResultDto> UpsertAsync(UpsertResultDto dto)
    {
        ...
        var resultDto = new ResultDto(upsert.Id, upsert.TeamId, upsert.Team?.Name ?? "", upsert.DisciplineId, upsert.Points, upsert.RawValue, upsert.UpdatedAt);
